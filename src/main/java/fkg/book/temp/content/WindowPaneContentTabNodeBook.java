 ...
 * @author MoeKagari
 */
public class WindowPaneContentTabNodeBook extends TableView<AbstractCharaData> implements WindowPaneContentTabNode {
	...
	public WindowPaneContentTabNodeBook(WindowPaneContent windowPaneContent) {
		...
		//不允许选择
		this.setOnMouseClicked(ev -> this.getSelectionModel().clearSelection());
		this.setOnKeyPressed(ev -> this.getSelectionModel().clearSelection());
		this.setOnKeyReleased(ev -> this.getSelectionModel().clearSelection());

		double normalWidth = 85;
	...
	private static Callback<TableColumn<AbstractCharaData, Image[]>, TableCell<AbstractCharaData, Image[]>> getIconCellFactory(WindowPaneContentTabNodeBook book) {
