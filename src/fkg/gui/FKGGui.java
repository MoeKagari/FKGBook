...
import fkg.server.proxy.ProxyServer;
...
public class FKGGui {
	...
	private static final SimpleDateFormat CONSOLE_TIME_FORMAT = new SimpleDateFormat("HH:mm:ss");
	...
	public FKGGui(String message) {
		...
		buttonComposite.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		{
			...
			allFKG.addSelectionListener(new ControlSelectionListener(ev -> {
				if (this.shower == null) {
					this.shower = new ShowFlowerInformation();
				}
				this.shower.display();
		...
		portGroup.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		{
			...
			serverPort.addSelectionListener(new ControlSelectionListener(ev -> {
				AppConfig.setServerport(serverPort.getSelection());
			}));
			...
			agentComposite.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
			{
				...
				agentPort.addSelectionListener(new ControlSelectionListener(ev -> {
					AppConfig.setAgentport(agentPort.getSelection());
				}));
				...
				useProxy.addSelectionListener(new ControlSelectionListener(ev -> {
					AppConfig.setUseProxy(useProxy.getSelection());
				}));
			}
			...
			apply.addSelectionListener(new ControlSelectionListener(ProxyServer::restart));
		}
