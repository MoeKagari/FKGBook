...
public class CharacterData {
	...
	public ImageIcon getIcon() {
		return new ImageIcon(getImage(this.id, ShowConfig.CHARACTER_ICON, ShowConfig.getCharacterIconNetpath(this.id), 50, 50));
	}

	public BufferedImage getStand() {
		return getImage(this.id, ShowConfig.CHARACTER_STAND, ShowConfig.getCharacterStandNetpath(this.id), 960, 640);
	}

	public BufferedImage getValentine() {
		try {
			return ImageIO.read(new File(ShowConfig.CHARACTER_VALENTINE + "\\" + this.id + ".png"));
		} catch (IOException e) {
			return null;
		}
	}
	...
	public BufferedImage getSkillImage() {
		...
		BufferedImage stand_s = getImage(this.id, ShowConfig.CHARACTER_STAND_S, ShowConfig.getCharacterStandSNetpath(this.id), 329, 467);
	...
	static {
		...
		for (int i = 0; i < 4; i++) {
			try {
				BufferedImage temp = ImageIO.read(new File(ShowConfig.SKILL_PANEL_BACKGROUND + "\\" + (i + 1) + ".png"));
