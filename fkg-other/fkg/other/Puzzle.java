...
public class Puzzle {
	...
	public static void main(String[] args) {
		try {
			deal(FileUtils.readFileToByteArray(new File(GetTurningCardSheet.filename)));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private static void deal(byte[] source) {
		...
		Json.createReader(new ByteArrayInputStream(source)).readObject().getJsonArray("turningCardSheetList").getValuesAs(JsonObject.class).stream().//
	...
	private static void deal(List<Card> levelResult, String filename) {
		...
		levelResult.sort(Comparator.comparingInt(obj -> obj.cardOrderNum));
