...
import fkg.masterdata.CharacterBook;
...
public class GetBook extends FKGApiHandler {
	...
	private static ArrayList<ArrayList<Integer>> getIDS() {
		...
		List<CharacterInformation> cis = GetMasterData.MASTERCHARACTER;
		for (CharacterInformation ci : cis) {
			if (ci.isCharacter()) {
				cid.add(ci.getId());
			}
		}

		List<CharacterBook> cbs = GetMasterData.MASTERBOOK;
		for (CharacterBook cb : cbs) {
			int id = cb.getId();
			if (cb.showInBook()) bid.add(id);
			if (cb.hadBloomed()) sbid.add(id);
		}
